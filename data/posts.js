import { users } from "./users"
export const posts=[
    {
        imageUrl:'https://scontent.fsap9-1.fna.fbcdn.net/v/t39.30808-6/273762314_489498142530768_4109661712658876568_n.jpg?_nc_cat=111&ccb=1-5&_nc_sid=8bfeb9&_nc_ohc=k0SXmrATu5AAX8bDFLw&_nc_ht=scontent.fsap9-1.fna&oh=00_AT8i545nriJ4toQxJiXC2G5b9e5I3z4ng_7QipRhooNjdA&oe=620D69EF',
        user:users[3],
        likes:1233,
        caption:'Ninja Turtles🐢🐢',
        comments:[
            {
                user:'Aya Ak',
                comment:'the best always 👑❤👑'
            },
            {
                user:'Mélïssa Dæyoü',
                comment:'My favorite ❤️'
            },
            {
                user:'Sondus Mahmoud',
                comment:'Do I really see an angel here?!! 💜🌆'
            }
        ]
    },
    {
        imageUrl:'https://scontent.fsap9-1.fna.fbcdn.net/v/t1.6435-9/144583984_3463258077129315_7669813250999303737_n.jpg?_nc_cat=103&ccb=1-5&_nc_sid=8bfeb9&_nc_ohc=PQj3pZyMWjcAX8AHqiz&_nc_ht=scontent.fsap9-1.fna&oh=00_AT-9G0VGSlD6u7Tc6LwdW3iE-W_kNao4KUggb8Lg0qKaZQ&oe=622D17E3',
        user:users[4],
        likes:14,
        caption:'Jesucristo es el SEÑOR! 🔥🔥🔥🔥🔥',
        comments:[
            {
                user:'Teresa Serrano',
                comment:'Amén 🙏 gloria a Jesús 🙏'
            }
        ]
    },
    {
        imageUrl:'https://scontent.fsap9-1.fna.fbcdn.net/v/t39.30808-6/263738779_1306618786472675_7490816667181208262_n.jpg?_nc_cat=106&ccb=1-5&_nc_sid=8bfeb9&_nc_ohc=EaI5d6Sa09oAX9tTsET&_nc_ht=scontent.fsap9-1.fna&oh=00_AT8RacMWjYY3VnsXCl1RggptpYC76YVVLO-xf1vJA_hYgw&oe=620C97BB',
        user:users[0],
        likes:23,
        caption:"There's no time to kill today!",
        comments:[
            {
                user:'Bill Leymeister',
                comment:'I have already done so once, and i have NEVER stopped living in the moment from that time on, i have little use for what the "Time" is! 🖤'
            },
            {
                user:'Roaring River Rodgers',
                comment:'I’m afraid. I’m afraid of leaving my kids and grandkids behind in this hateful world alone'
            }
        ]
    },
    {
        imageUrl:'https://pbs.twimg.com/media/FLNWfNpVkAUL65K?format=jpg&name=large',
        user:users[2],
        likes:5030,
        caption:'Tower arms are lifting Starship onto Super Heavy Booster',
        comments:[
            {
                user:'Crypto Lion',
                comment:"And here in South Korea government banned all foreigners from using crypto currency.  If your foreigner in South Korea they I’ll freeze all your crypto accounts and won’t let you withdraw your own money."
            },
            {
                user:'Zobeer',
                comment:"Isn’t crypto decentralized, the only case they can do that is on centralized exchanges"
            }
        ]
    }
]